...
> This article demonstrates how to effectively use the `langchain-dev-utils` library in a `langchain` and `langgraph` project through a **multi-agent example**. This example deeply integrates all core modules of this library, helping you comprehensively master the practical usage of the library's core components.

## Process Description

This multi-agent architecture performs a complete analysis of a user-input topic theme. A main agent decomposes the topic into several subtopics and distributes them to multiple sub-agents for parallel processing. Each sub-agent is responsible for analyzing its assigned subtopic and recording the analysis results as notes. Subsequently, a large language model summarizes the analysis reports generated by all sub-agents to form a final summary report.
...
- Each sub-agent (Talker) performs its analysis task and writes the results into a note.
...
- `deepseek`: Integrated via `langchain-deepseek` (It is an officially `init_chat_model` supported model provider, no need to register again).
- `qwen`: Integrated via `langchain-qwq` (Needs registration, pass `ChatQwen` to `chat_model`).
- `kimi` and `glm`: Integrated using the `OpenAI Compatible` style (Need registration, but no suitable integration library exists; however, the providers support OpenAI-Compatible APIs, pass `openai-compatible` to `chat_model`).
...
- `transfor_to_talk`: A routing agent that delegates the user's request to multiple agents for discussion.
- `ask_human_for_more_details`: Requests more details about the topic from the user.
...
Using the `has_tool_calling` and `parse_tool_calling` functions from [Message Processing](./message-processing.md):
...
Use the `transfor_to_talk` tool to decompose the topic "{topic}" into {num} subtopics and have the discussion results for each subtopic written into notes.
...
You have permission to use the `query_note` and `ls` tools. If you want to emphasize the discussion results of a particular subtopic, you can use the `query_note` and `ls` tools.
```
...
Additionally, there is a tool for writing notes:
...
write_note = create_write_note_tool(
    name="write_note",
    description="""Used to write a note.
...
Each discussion agent is built using the [Prebuilt Agents](./prebuilt.md).
...
Then, for these sub-agents, use [State Graph Orchestration](./graph-orchestration.md) to build a parallel agent pipeline, implementing user-specified discussion participants through the `branches_fn`.
...
After completion, you must use the `write_note` tool to write the final result into a note.
```
...
Here, the `message_format` function from [Message Processing](./message-processing.md) is used to format the list of strings into a single string.
